 ...
 */
function template(array $attributes = []): Template
{
...
describe('template rendering', function (): void {
    ...
    it('refuses a dotted path that is not declared', function (): void {
        $this->renderer->render(
            template(['body_template' => 'Bonjour {{ order.customer.name }}.']),
            ['order_number' => 'CMD-1', 'customer_name' => 'Marie'],
        );
    })->throws(TemplateRenderingFailed::class, 'order.customer.name');

    it('refuses a path deeper than four segments', function (): void {
        $this->renderer->render(
            template([
                'body_template' => '{{ a.b.c.d.e }}',
