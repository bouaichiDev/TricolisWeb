...
describe('destinations d’export', () => {
  ...
  /**
   * §32 : CSV et PDF n'ont pas de générateur pour une facture. Les proposer
   * créerait une destination qui échoue à chaque clôture, loin de cet écran.
   */
  it('ne propose que les formats réellement produits', async () => {
    render()
  ...
  it('déclare le type et la fréquence que la clôture consulte', async () => {
    ...
    await userEvent.type(within(dialog).getByLabelText('Hôte'), 'https://coop.example')
    await userEvent.click(within(dialog).getByRole('button', { name: 'Enregistrer' }))
